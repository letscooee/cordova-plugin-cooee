...
/**
 * Hook to add Swift support to an iOS project.
 * It will be called before adding Cooee Cordova plugin.
 */
const fs = require('fs');
...
module.exports = context => {
    ...
    if ((context.hook === 'after_platform_add' && context.cmdLine.includes('platform add')) ||
        ...
        (context.hook === 'before_plugin_add' && context.cmdLine.includes('plugin add')) ||
        (context.hook === 'before_plugin_install' && context.cmdLine.includes('plugin add')) ||
        (context.hook === 'before_plugin_install' && context.cmdLine.includes('platform add'))) {
        getPlatformVersionsFromFileSystem(context, projectRoot).then(platformVersions => {
            const IOS_MIN_DEPLOYMENT_TARGET = '13.0';
